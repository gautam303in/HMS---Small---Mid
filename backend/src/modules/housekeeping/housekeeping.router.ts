...
housekeepingRouter.patch('/housekeeping/tasks/:id/clean-status', (req: Request, res: Response) => {
  const task = store.housekeepingTasks.find(t => t.id === req.params.id || t.roomNumber === req.params.id);
  ...
  store.logAudit('Housekeeping Staff', 'Attendant', 'HOUSEKEEPING_STATUS_UPDATE', `Room ${task.roomNumber} updated to ${cleanStatus} (${availability})`);
  res.json(task);
});
