...
export const DashboardView: React.FC<DashboardViewProps> = ({ onNavigateTab }) => {
  ...
  const handleCompleteCheckIn = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      await fetch(`http://localhost:5000/api/reservations/${selectedCheckInGuest}/checkin`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
  ...
  const handleCompleteCheckout = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      await fetch(`http://localhost:5000/api/reservations/${selectedCheckoutRoom}/checkout`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
